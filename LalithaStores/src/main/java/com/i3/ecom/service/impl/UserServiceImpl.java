...
public class UserServiceImpl implements UserService {
	...
	public ResponseMessage addUser(final Users userJson) {
		...
		try {
			...
			userDao.setUserRoles(getUserRoleString(userJson.getRoles()),
					user.getUserId());
	...
	public ResponseMessage getAllUsers() {
		...
		try {
			...
			roles = userDao.getAllUserRoles();

			for (int i = 0; i < users.size(); i++) {
				if (users.get(i).getUserId().equals(roles.get(i).getUserId())) {
					users.get(i).setRoles(
							Arrays.asList(roles.get(i).getRole().split(",")));
				}
			}
	...
	public UserDetails loadUserByUsername(String email)
		...
		try {
			...
			if (roles != null) {
				...
				loggedInUser = new LoggedInUser(user.getEmailId(),
